...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* second_smallest = removeSmallest(singleQueue, mergeQueue);

        int freq = smallest->freq.getFrequency() + second_smallest->freq.getFrequency();
        TreeNode* merged = new TreeNode(freq);
        merged->left = smallest;
        merged->right = second_smallest;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    TreeNode* root = NULL;
    if (bfile.getNextBit()) {
        root = new TreeNode(Frequency(bfile.getNextByte(), 0));
    } else {
        root = new TreeNode(0);
