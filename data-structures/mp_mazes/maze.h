...
class SquareMaze {
    private:
        ...
        std::vector<std::vector<bool>> down_walls_;
    public:
        /**
         ...
         * existing data before doing so. You will start with a square grid
         * (like graph paper) with the specified height and width. You will
         * select random walls to delete without creating a cycle, until there
         * are no more walls that could be deleted without creating a cycle.
         ...
         * @param x The x coordinate of the current cell
         * @param y	The y coordinate of the current cell
         * @param dir	Either 0 (right) or 1 (down), which specifies which
         * wall to set (same as the encoding explained in canTravel).
         * You only need to support setting the bottom and right walls of
         ...
         * distance from the origin (i.e. the top-left cell), which is defined
         * as the length (measured as a number of steps) of the only path through
         ...
         * except the entrance (1,0) through (9,0). For each square in the maze, call
         * its maze coordinates (x,y). If the right wall exists, then blacken the
         * pixels with coordinates ((x+1)*10,y*10+k) for k from 0 to 10. If the bottom
         * wall exists, then blacken the pixels with coordinates (x*10+k, (y+1)*10) for
         * k from 0 to 10.
         * 
         ...
         * (Red is 0,1,0.5,1 in HSLA). Then if the second step is right, color
         * pixels (5,15) through (15,15) red. Then if the third step is up, color
         * pixels (15,15) through (15,5) red. Continue in this manner until you
         * get to the end of the solution vector, so that your output looks
