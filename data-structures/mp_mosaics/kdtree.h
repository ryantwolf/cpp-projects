...
class KDTree
{
  ...
  private:
     ...
     */
    void clear_node(KDTreeNode *subroot);
    void clear();
    void copy_recurse(KDTreeNode *subroot, KDTreeNode *other_subroot);
    void copy(const KDTree<Dim>& other);
    double distance(const Point<Dim>& first, const Point<Dim>& second) const;
    size_t partition(vector<Point<Dim>>& points, size_t left, size_t right, size_t pivotIndex, int curDim) const;
    Point<Dim> select(vector<Point<Dim>>& points, size_t left, size_t right, size_t k, int curDim) const;
    KDTree::KDTreeNode* buildTree(vector<Point<Dim>> points, int curDim) const;
    Point<Dim> searchTree(const Point<Dim>& query, KDTree::KDTreeNode* current, int curDim) const;
};
