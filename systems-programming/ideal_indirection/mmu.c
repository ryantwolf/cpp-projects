...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    // If pid is different, then there was a context switch
    // "Use pid to check for a context switch. If there was a switch, flush the TLB"
    // if (pid != this->curr_pid) {
    //     tlb_flush(&this->tlb);
    // }
    // // Might need to change this->curr_pid

    // //"Make sure that the address is in one of the segmentations.
    // // If not, raise a segfault and return"
    // if (!address_in_segmentations(segmentations[pid], virtual_address)) {
    ...
    // // addr32 base_pd_address = *pd >> 12;
    // // addr32 pd_offset = virtual_address >> 22;
    // // addr32 base_pagetable_addr = (virtual_address >> 12)
