...
A library call is a regular function while a system call is an operation the kernel deals with. printf is a library call and write is a system call.

2.	What is the `*` operator in C? What is the `&` operator? Give an example of each.

`*` will dereference a pointer (get the value stored at the address the pointer points to) while `&` will get the address of a variable.

3.	When is `strlen(s)` != `1+strlen(s+1)` ?

When s points to at least 2 null characters.

4.	How are C strings represented in memory? What is the wrong with `malloc(strlen(s))` when copying strings?

C strings are character arrays that are terminated by the null character. The `malloc` call does not allocate enough memory for the null byte.
...
First, the first index is used in the first lookup table to find the address of the next table.
With the second page table, the second index is used to look up the frame. Then, the offset of the adress is used to access the exact piece of memory.
...
The MMU is the general unit that performs memory lookups. The TLB stores recently accessed memory addresses.
...
Everything is shared except for the stack.
...
A process can be orphaned if its parent process ends before it does. The process will eventually be taken care of with the init process, which waits on everything.
...
The time quantum is up and another process has (pre-emptive) priority.
...
Circular wait means that there is a cycle in the resource allocation graph, mutual exclusion means that no two processes can share the same resource, hold and wait means that a process keeps a resource locked once it has it, and no-preemption means that a process cannot force another process to give up its resource.
...
All signals except SIGSTOP and SIGKILL.
...
Socket creates a file descriptor for the server. Bind aquires the port for the server. Listen starts listening on the port. Accept creates file descriptors for the clients that connect to the port.
...
TCP ensures that all the packets arrive but UDP does not.
...
```C
// Symbolic Link
symlink("/etc/password", "symlink");
// Hard link
link("/etc/password", "link");
```
...
The size, permissions, and blocks of the file are stored in an i-node. 
...
The information would be different if `path` is a symbolic link.
...
Why is `sizeof(char)` unnecessary? Answer: a `char` by definition is only 1 byte.