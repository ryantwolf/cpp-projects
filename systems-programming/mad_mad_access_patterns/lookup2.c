...
int main(int argc, char **argv) {
  ...
  char *file_data = mmap(NULL, file_stat.st_size, PROT_READ, MAP_PRIVATE,
                         file_descriptor, 0);
  ...
  for (int i = 2; i < argc; i++) {
    BinaryTreeNode *node = (BinaryTreeNode *)(file_data + BINTREE_ROOT_NODE_OFFSET);
    BinaryTreeNode *found_node = NULL;
    while (node->left_child != 0 || node->right_child != 0) {
      // printf("%s: %d, $%.2f\n", node->word, node->count, node->price);
      if (strcmp(argv[i], node->word) < 0) {
        if (node->left_child == 0) {
          break;
        }
        node = (BinaryTreeNode *)(file_data + node->left_child);
      } else if (strcmp(argv[i], node->word) > 0) {
        if (node->right_child == 0) {
          break;
        }
        node = (BinaryTreeNode *)(file_data + node->right_child);
      } else {
        found_node = node;
        break;
      }
    ...
    if (found_node == NULL) {
      ...
      printFound(argv[i], found_node->count, found_node->price);
    }
  }
  return 0;
}
